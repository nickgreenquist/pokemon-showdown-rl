...
`rl/search/matrix.py:202-205` wraps `calculate_damage` in `except BaseException: dmg = None`. A Ctrl-C or `SystemExit` during a search arm degrades into "no damage attribution" instead of stopping. Fix: `except Exception`.
