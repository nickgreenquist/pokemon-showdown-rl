"""DEEP SEARCH Step C's enabler -- the TREE-OP inside the engine collector
(`rl/search/tree_top.py`), on the R7 fleet's critic form (the plain observation critic,
plan box 5 item 5), in process and in the child process: the T-op's collector contract
(tests/test_top_collector.py) on the tree.

What must hold:
  * every episode carries `search_mask` / `search_pi` / `search_v` aligned to its rows;
    pi' is a masked distribution on searched rows and zero elsewhere; the dose lands
    (searched fraction of the eligible in a band under frac 0.5); forced and confident
    rows are never searched;
  * PLAY: on searched rows the played action is a legal support point of pi' and
    `old_logp` is EXACTLY float32(log pi'(a)); on unsearched rows it is the policy's own;
  * THE RATIO IDENTITY ON REAL ROWS: at version 0 the learner's epoch-0 ratio is exactly
    exp(log pi_theta(a) - log pi'(a)) on searched rows and ~1 on the rest; under
    RECORD-ONLY (`play: false`, Step C's default) ~1 everywhere while searched rows exist
    with argmax pi' != argmax pi_theta;
  * the counters reach the collector's stats (`search/*` and the tree's dose `tree/*`),
    and the child-process collector picks the TreeOp from the block (`searcher_class`)
    and carries the same keys.

No server. Skips loudly without the built extension or a team bank (the worktree's own
data/, else the main checkout's).
"""
...
AGENT = dict(algo="ppo", lr=2.5e-4, gamma=1.0, gae_lambda=0.95, rollout_steps=8, epochs=1, minibatches=1,
             ...
             trunk="entity_deepsets", trunk_kwargs=TRUNK_KWARGS, antisymmetric_critic=False,
             privileged_dim=0, search_targets=True, search_policy_coef=0.1)
DIALS = dict(frac=0.5, top1_skip=0.97, tree={"sims": 64, "mode": "br_prior", "root_rule": "gumbel_mctx", "batch": 2})
cfg = Config(env_id="ShowdownGen1-v0", seed=11, total_steps=1000, eval_every=10**9, eval_episodes=1,
             run_name="tree_top_test", num_envs=1, agent=AGENT,
...
# The dial list is the signature; `play` has no default.
assert set(TreeOp.dials()) == {"frac", "top1_skip", "tree", "play"}, TreeOp.dials()
TreeOp.check_dials({**DIALS, "play": True})
try:
    TreeOp.check_dials(DIALS)
...
def collect(play, steps=1500, seed=4242):
    c = EngineCollector(agent.act_logp, pool, seed=seed, k=16, team_bank=BANK, privileged=False, both_views=False)
    c.searcher = TreeOp(agent, c.tables, seat="p1", seed=1, play=play, **DIALS)
    c.seam.version = 0
    c.start(10_000)
...
for key in ("search/searched_frac", "search/eligible_frac", "search/decisions", "search/kl_prior",
            ...
            "tree/sims", "tree/depth_mean", "tree/nodes", "tree/batch_rows", "tree/errors", "tree/fallback"):
    assert key in st and np.isfinite(st[key]), (key, st.get(key))
assert st["search/kl_prior"] > 0 and st["search/leaves"] > 0
assert st["tree/depth_mean"] > 1.0 and st["tree/errors"] == 0 and st["tree/fallback"] == 0, st
assert st["tree/batch_rows"] >= 1.0, st
...
pc = ProcCollector(asdict(cfg), agent, pool, seed=4243, k=16, team_bank=BANK, privileged=False, both_views=False,
...
assert "tree/depth_mean" in stp and "tree/batch_rows" in stp, sorted(stp)
pc.close()
print(f"OK tree-op collector: {sum(len(e['actions']) for e in eps)} child rows, searched frac {stp['search/searched_frac']:.2f}")
"""


@pytest.mark.skipif(not BANKS, reason="no team bank built yet")
def test_the_tree_op_searches_records_and_plays_pi_prime_and_the_learner_reads_it():
    r = subprocess.run(
