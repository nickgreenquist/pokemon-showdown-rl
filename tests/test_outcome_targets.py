...
def test_engine_collector_emits_the_block_only_when_asked():
    ...
    raw = {"length": 4, "reward": 1.0, "obs": _episode(last, n=4),
           ...
           "old_logp": np.zeros(4, np.float32), "version": np.zeros(4, np.int64)}
    off = SimpleNamespace(_opp_action=False, _privileged=False, _outcome_targets=False)
    on = SimpleNamespace(_opp_action=False, _privileged=False, _outcome_targets=True)
