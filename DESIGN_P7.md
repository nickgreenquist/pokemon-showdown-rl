...
## 10. THE HUMAN REPLAY CORPUS — verified, and it may dominate this package

**Status: newly surfaced 2026-08-04, verified the same evening. Phase placement is OPEN and is the
principal question for this review.** §9.7 previously said "nobody has checked." Someone has now.

### What exists

`HolidayOugi/pokemon-showdown-replays` on HuggingFace — public Showdown replays pulled via the
Showdown API. 33,154,470 rows / 69.8 GB parquet, upload dates 2005–2026, counts current to
2026-06-20. Schema: `id`, `format`, `players`, `log` (full battle log), `uploadtime`, `views`,
`formatid`, `rating`.

**`gen1randombattle`: 109,147 replays** — verified against the dataset's own per-format table, and
more than Gen 1 OU's 102,574. At Gen 1's long games and counting both players' perspectives this
is plausibly 10–20M state-action pairs, i.e. one to two orders of magnitude beyond P4's 903,090
SH decisions. Filter to the format rather than pulling 69.8 GB.

### Why it matters more than anything in §4

**Human demonstrations are not bounded by 0.489.** Every arm in §4 is capped by the SH-imitation
ceiling (§3). Metamon measured SH against the human ladder at Gen1OU at **16W–59L ≈ 0.21** — the
ladder population beats SH roughly 4 times in 5. A clone of that population is a fundamentally
different lever from a clone of SH, and it is the lever VGC-Bench actually measured at +25–30.

**Temper it:** `rating` is **mostly null** in this corpus (verified in the sample rows), so there
is no skill filter and a clone learns the *mean uploader*, not the strong players inside that 0.21
figure. Expect well above SH, not the naive +0.3.

### What it costs — the shortcut does NOT exist

The hope was to reuse Metamon's parser and skip reconstruction. Checked, and it does not work:

- **Metamon does not support random battles.** Gens 1–4 **OU** plus Gen 9 OU; `gen1randombattle`
  is not a supported format string.
- **It no longer emits poke-env objects.** Metamon replaced poke-env's message parsing with its
  own parser, so its output cannot be fed to our existing `embed_battle(battle, type_chart)` —
  which was the one genuinely cheap integration path.
- **The action spaces do not map trivially.** Metamon: 13 discrete (moves 0–3, switches 4–8,
  gimmicks 9–12) with a 9-choice `MinimalActionSpace` for Gens 1–4, and **moves and switches in
  ALPHABETICAL order**. Ours: poke-env insertion order (`list(battle.team.values())`,
  `list(active.moves.values())[:4]`), 6 switch slots including the always-illegal self-switch. A
  re-sort plus a hole, not a permutation. **An off-by-one here silently mislabels every row in the
  dataset** — the exact failure class pokejax's diagnosed obs-bridge bug list warns about, and it
  would be invisible in training curves.
- **Spectator replays hide the player's private view.** Metamon's own docs concede they use
  "heuristic predictors and usage statistics" and that "there is no way to be perfect." Their
  reconstructed trajectories carry `action = -1` where a choice was never revealed; those rows need
  an explicit policy (masking them out of the loss, most likely).

So this is a **parser fork plus an action-mapping correctness problem plus a data pipeline**,
before a single training run. Their Gen-1 *mechanics* parsing is still the right thing to fork —
gen1randombattle and gen1OU differ only in team source.

### One structural advantage nobody had noticed

**Hidden-state reconstruction is easier in randbats than in OU.** Metamon needs usage-statistics
heuristics because OU teams are player-chosen from an enormous space. `gen1randombattle` sets are
generated by the server from a **fixed, public, enumerable pool**, and this repo already vendors
`showdown/data/random-battles/gen1/teams.ts`. The belief-state problem is strictly better posed
for our format than for theirs — the opposite of the standing worry that choosing randbats locked
us out of the human-data lever.

### Other caveats to price
...
Generate demonstrations from a search-based bot (e.g. Foul Play, open source, #8 in Gen1OU at
PokéAgent 2025 — well above SH) instead of parsing humans. Perfectly formatted, correct
perspective, our exact action space, zero parsing. **But "unlimited" understates the compute
badly:** P4 generated SH data at 2,769 decisions/s, while search agents run orders of magnitude
slower per decision (Wang's MCTS ≈ 10 s/move). P4-scale data from a search bot is plausibly days,
not minutes. It is also unverified whether Foul Play supports `gen1randombattle`. Lower ceiling
than human experts, far lower engineering risk.
...
An afternoon, not a chapter, and it converts this from a hunch into a priced option. Pull a sample
of the 109k `gen1randombattle` replays and establish:

1. what fraction parse cleanly;
2. the real decision count per battle after dropping spectator-hidden actions (the `-1` rows);
3. whether **any** rating/skill signal survives, since the ceiling argument depends on it;
4. whether the fixed randbats set pool makes team inference tractable, as §10 argues it should;
5. a measured win rate for the uploader population vs SH, if inferable from the logs — this is the
   number that would actually bound a human-BC ceiling, replacing the borrowed Gen1OU 0.21.
