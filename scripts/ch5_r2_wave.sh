#!/bin/bash
# CH5 R2 wave driver — STRICTLY SERIAL, k=1. COPY of scripts/ch5_r1_wave.sh
# (the repo's copy-not-edit precedent: R1's committed artifacts were
# produced by that file) with exactly these deltas: PREREG/OUT/ARMS point
# at R2, the provenance heredoc's prereg path (a LITERAL that would
# otherwise stamp R1's sha into R2's provenance — designer B trap 4), the
# grader name, and stall_polls_for grown a R4S* case (R2's search arms
# match neither B* nor RS*, and the greedy default would give a search
# seat half its stall budget).
#
...
# in $OUT/wave.log and scripts/ch5_r2_grade.py asserts they do not overlap.
#
...
PREREG="configs/eval/ch5_r2_offsh.yaml"
...
wlog "WAVE COMPLETE"
wlog "G2 OWED on every arm: scripts/ch5_r2_grade.py before any number is quoted."
