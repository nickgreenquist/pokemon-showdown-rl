...
For each 50M lane, forwards that lane's own final-policy self-play obs
(d22_collect_obs.py output) through actor and critic at six checkpoints,
and measures:
...
LANES = (35, 36, 37)
STEPS = (500_000, 6_000_000, 12_000_000, 25_000_000, 37_500_000, 50_000_000)


def build(part: str, trunk_kwargs: dict) -> EntityDeepSetsNet:
...
def probe(net: EntityDeepSetsNet, obs: torch.Tensor) -> tuple[dict, dict]:
    ...
    for name, key in (("ctx_out", "ctx"), ("mon_net.3", "mon")):
        m = acts[name].reshape(-1, acts[name].shape[-1])
...
def main() -> None:
    ...
    for seed in LANES:
        obs = torch.from_numpy(
            np.load(out / f"obs_s{seed}.npz")["obs"])
    ...
    print("\n=== dormant fraction tau=0.025 (actor), by layer, 500k -> 50M ===")
    for seed in LANES:
