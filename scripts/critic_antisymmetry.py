...
def main() -> None:
    ...
    print("\n  HOW TO READ IT  [rewritten 2026-09-19 -- the original reading")
    print("  was WITHDRAWN by RESULTS §31's re-scope box]:")
    print("   * per-side bias >> 0  -> the critic is optimistic about WHICHEVER")
    print("     side it is pointed at. Since p1/p2 are symmetric this is NOT a")
    print("     seat effect: the mechanism is the TRAIN/EVAL DISTRIBUTION SHIFT")
    print("     league play creates (pool of older, weaker checkpoints).")
    print("   * per-side bias ~ 0   -> the critic is self-consistent, and §27.1's")
    print("     optimism must come from somewhere else.")
    print()
    print("  WHAT THIS PROBE CANNOT DO, and the published version got wrong: it")
    print("  cannot EXONERATE the determinization sampler. V(s) and V(swap(s))")
    print("  come from the SAME determinization -- which is exactly why no")
    print("  sampler enters -- so the probe carries ZERO information about it.")
    print("  A probe that holds a factor fixed says nothing about that factor.")
    print("  Nor does it license 'this fully accounts for §27.1's +0.0416':")
    print("  different position sets, different turn mixes, and the bias below")
    print("  varies 5x with the turn.")
    print("\n  It measures SELF-CONSISTENCY, never accuracy. §27.1's ranking gap")
    print("  (~93% of the distance to the ceiling) is untouched either way.")
    print("=" * 78)
