#!/bin/bash
# CH5 100M PRIMARY wave driver — STRICTLY SERIAL, k=1. COPY of
# scripts/ch5_r2_wave.sh (the repo's copy-not-edit precedent) with exactly
# four literal deltas, diff-verified at creation: PREREG/OUT/ARMS point at
# the 100M primary (configs/eval/ch5_100m_offfp.yaml, results/ch5_100m,
# T104 T112 T120) and the grader name is ch5_100m_grade.py. Everything
# else — the four FP incident fixes via ch3_r4_fp_runner.sh, ops-failure
# sentinels, provenance stamping, resume-safe skip — is R2's, unchanged.
# stall_polls_for keeps the R4S* case; no such arm exists here (greedy
# only), so every arm gets the 30-poll greedy budget.
#
#   bash scripts/ch5_r2_preflight.sh && bash scripts/ch5_r2_wave.sh
#
...
# in $OUT/wave.log and scripts/ch5_100m_grade.py asserts they do not overlap.
#
...
PREREG="configs/eval/ch5_100m_offfp.yaml"
...
print(json.dumps({
  ...
  "prereg_sha256": sha("configs/eval/ch5_100m_offfp.yaml"),
}, indent=2))
PYEOF
...
stall_polls_for() {
    case "$1" in
        R4S*) echo 60 ;;   # R2's search seats; B*/RS* were R1's spelling
        *)    echo 30 ;;   # T* greedy arms
    esac
}

# wave_plan.order: the three T arms, serial. The single R4S arm (Q6
# selection rule: lowest-numbered SURVIVING treatment seed) is launched
# separately AFTER the T arms via ARMS="R4S<seed>" — it is not in the
# default list because which one runs is not known until survival is.
ARMS="${ARMS:-T104 T112 T120}"
...
wlog "WAVE COMPLETE"
wlog "G2 OWED on every arm: scripts/ch5_100m_grade.py before any number is quoted."
