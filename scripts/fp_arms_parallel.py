...
(scripts/ch3_r4_fp_runner.sh) -- the FP-parallel task's Phase 2 (2026-09-25).
    ...
    /opt/anaconda3/bin/python scripts/fp_arms_parallel.py --prereg configs/eval/X.yaml \\
        --arms A,B,C,... --slots 8 [--out results/X] [--fpdir ../foul-play-fpprobe]
...
scripts/r6_reads_queue.sh. Writes <out>/parallel.log and <out>/parallel_summary.json.
"""
...
def main():
    ...
    # moment before launch, and a wall-clock arm's read needs the whole run.
    contamination = []
    ...
    base.update(STALL_POLLS="60", MAX_RELAUNCHES="10", NO_PROGRESS_RELAUNCHES="3",
    ...
    def seat_json(a):
        tag = a.lower() if not args.smoke_battles else f"smoke_{a.lower()}"
        return Path(prereg["results_dir"]) / f"{tag}.json"
    ...
    log(f"START pid {os.getpid()} slots={args.slots} arms={queue} fpdir={base.get('FPDIR', '(runner default)')} "
        f"budget={'FP@N' if not wallclock else 'wall-clock'}")
    last_launch = 0.0
    try:
                ...
                for pid, r in rows.items():
                    ...
                    if use >= 0.5:
                        contamination.append({"t": utc(), "pid": pid, "cores": round(use, 2),
                                              "cmd": r[3][:120]})
                        log(f"CONTAMINATION: pid {pid} at {use:.2f} cores: {r[3][:100]}")
