...
def header(*, arm: str, lane: int | None, donor: dict, all_donors: list[dict], base: str, lr: float, lr_evidence: str,
    ...
    return [
        ...
        "#   other work -- then the OBJECT RULE's reads, then the BIG-BUDGET leg on FP@500's own CALIBRATED FP@N (never serial), then the",
        ...
        *power_lines(power, width),
        ...
        "#   \"3 seeds pooled\"); the BIG-BUDGET leg on ENS3-S and ENS3-C (the plan's G4) in the primary's policy form (greedy, loop breaker",
        "#   off), n 250 each (the FP budget ladder's precedent n;",
        "#   ~45 min in all K-wide on the calibrated FP@N (it was ~2.7 h an arm serial); visits-matched, strength not tested; se_diff ~0.045, DESCRIPTIVE only); per-arm",
        ...
        "#   read (the 09-22 principle, a ladder run is EARNED offline; the bar, ratified 09-24, RESTATED 09-25: >= +0.05 off FP@N over the",
        "#   re-drawn previous object, both re-drawn in one session -- re-derived on FP@N 25k/12k, never translated from FP@20).",
        "#",
