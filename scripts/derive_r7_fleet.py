...
def header(*, arm: str, lane: int | None, donor: dict, all_donors: list[dict], base: str, lr: float, lr_evidence: str,
    ...
    return [
        ...
        *power_lines(power, width),
        ...
        "#   ~1-1.5 h each), after the primary in the same quiet session; per-arm loss/grad_norm, the clip_frac split and the",
        "#   realized dose (above).",
        ...
        "#          coefficient, tau: one lap; plan section 10 also names the student's capacity, but the attention screen",
        "#          already read as not clearing, section 34, so capacity is not the next lap). Never \"search does not work\".",
