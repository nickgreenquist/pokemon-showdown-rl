...
def header(*, arm: str, lane: int | None, donor: dict, all_donors: list[dict], base: str, lr: float, lr_evidence: str,
    ...
    return [
        ...
        "#       over those rows) -- beta moves the student toward pi' beyond what playing pi' and the v' head do. The LARGEST",
        ...
        "#       tests/test_engine_bank_mmap.py PASSES (not skipped), tests/test_lop.py (none skipped), tests/test_derive_r7_fleet.py",
        "#       and tests/test_r7_mechanism_reads.py (the mechanism instrument reproduces G0 and the evaluator) pass; every",
        ...
        *power_lines(power, width),
        ...
        "#   \"3 seeds pooled\"); FP@500 on ENS3-S and ENS3-C (the plan's G4), n 250 each (the FP budget ladder's precedent n;",
        ...
        "#          (i) and (vi) BOTH MOVED -> the lever STAYS in the base; the next lap is the STRONGER EVALUATOR (the mechanism",
        f"#            works below this fleet's power: P(X-POS) is {at20['X-POS']:.2f} at a true +0.020).",
        "#          Never \"search does not work\".",
        "#   THE CHANNEL a lap targets after X-COST or X-NEG: (i) not moved -> the policy target; (i) moved, (vi) not -> the",
        "#   evaluator; the searched arm's loss/clip_frac_searched above twice the control's -> the behaviour channel (off-policy",
        "#   data from a true-world search). The mechanism reads are reported beside every cell; outside X-FLAT, X-COST and X-NEG",
        "#   they change no action. LANE LOSS is the named cell under the R0 gates.",
