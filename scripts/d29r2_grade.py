...
surviving treatment lanes VOIDS the primary.
    ...
    python scripts/d29r2_grade.py                # after results/d29r2/final_s8*.json
    python scripts/d29r2_grade.py --selftest     # known-answer checks, no run files
"""
...
def d_a_lr_trace(seeds) -> bool:
    ...
    ok = True
    for s in seeds:
        run = Path(f"runs/showdown_sp_stack50m_r2_s{s}")
...
def main() -> None:
    ...
    rates, live, r04 = [], [], []
    for s in SEEDS:
        path = Path(f"results/d29r2/final_s{s}.json")
        if not path.exists():
