...
"""Pick HD1/HD2's margin_delta by matching D1's REALIZED override rate.
    ...
    python scripts/gate_r5_pin.py [--commit]
...
PREREG = REPO / "configs/eval/gate_r5.yaml"
RES = REPO / "results/gate_r5"
TARGET = 0.163
GRID = {"critic": {"C1A": 0.05, "C1B": 0.07, "C1C": 0.03},
        "heuristic": {"F1A": 0.10, "F1B": 0.12, "F1C": 0.08}}
...
def pick(cells):
    ...
    for tag, delta in sorted(cells.items(), key=lambda kv: kv[1]):
        ...
        fired = d.get("heuristic/fired_rate", 1.0) if d.get("search_heuristic") else 1.0
        if rate is None:
...
def main():
    ...
    args = ap.parse_args()
    print(f"target override rate {TARGET} (D2N realized)\nour critic:")
    b1 = pick(GRID["critic"])
    print("their heuristic:")
    b2 = pick(GRID["heuristic"])
    text = PREREG.read_text()
    for holder, best in (("PINC", b1), ("PINF", b2)):
        if holder in text:
    ...
    if args.commit:
        ...
        subprocess.run(["git", "commit", "-q", "-m",
                        f"gate_r5: match override rate — HD1 delta {b1[0]:.2f} "
