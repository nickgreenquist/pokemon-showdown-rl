...
3. **The live lever is the CRITIC, and §27/§27.1/§31 say what is wrong with it**: 93% of
   ...
   `docs/proposals/R7_NATIVE_SEARCH_PLAN_2026-09-22.md`, frozen at amendment box 3 until G0 reads. R7 B0–B3 + B1 BUILT 09-23 00:45Z on branch `r7-native-search` (worktree `../pokemon-showdown-rl-r7`, fresh env `pkmn-engine-r7`; commits c383d8e..4e30c0a, all tests green; ruling 7 defaulted to 5 lanes): the merge to main, the B0 bench (pass line 3.6 ms p99 six-wide) and G0 (`scripts/rollout_q.py`) all WAIT for the idle box after the R6 reads queue (Fri 09-25 ~05:00 EDT) — a running block imports the working tree, and no R7 job runs beside an FP arm.**
