...
CLOSED. **LADDER R1 PRIMARY READ IS MEASURED (corrected 2026-08-26): GXE
59.6%, Glicko-1 1573 +/- 27, final Elo 1292, 0.475, n=200, NOT listed.**
...
n=200 DESCRIPTIVE, GXE 59.6%, Elo 1292** · off-FP@20 (12M only) fleet **0.34867**
...
2. **OPEN FOR YOU: A-BR-1** (buy a 4th 50M lane? designer A says NO) and
...
- **RULED: R1-C funds BOTH rosters, E7 last; CHAPTER5 §3 C1 RETRO-RATIFIED
  at 0.0735; at k <= 2 the fleet-mean read is DESCRIPTIVE ONLY** (1 df: CI
  multipliers 0.45x-31.9x, a factor of 72).

## Watch items
- **"SCALE IS FLAT" WAS NEVER ESTABLISHED, ON EITHER AXIS.** 50M vs SH is
  0.7423/0.7347/**0.6297**; delta **0.44 se** vs a 0.0735 bar. Heterogeneity
  IS licensable: **F = 31.7 on (2,3) df, p = 0.0096.**
- **TWO GATES OPEN AND NAMED: G1** (no per-arm smoke budgeted) and **G-SEARCH**
  (nothing catches a search arm degrading to greedy on ~1/3 of its budget).
- **ATTENTION: the 34.6x was vs the FLAT MLP**, not production; attention-vs-
  `entity_deepsets` was NEVER measured. **Temporal context** is the sharper gap.
- **STYLE IS NOT THE GAP**: sum-|delta| US 0.095 / SH 0.095 / clone 0.124,
  gross move errors us 0.6% vs humans 2.7%; only the VOLUNTARY switch cut
  differs (6.9/10.7), ours REACTIVE. Critic fine too (AUC 0.891).
- **"NOT LISTED" NEVER MEANT "NO RATING".** The top-500 leaderboard JSON has
  only listed accounts; the USER PROFILE carries GXE/Glicko for any rated
  one. We polled the leaderboard and declared our own primary read
  unmeasurable for 2 days. Also: `L2.battles.jsonl.rating` is PRE-BATTLE, so
  the long-quoted "Elo 1311" was the second-to-last value; final is 1292.
