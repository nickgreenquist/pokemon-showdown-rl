...
2. **THE CALL IS CLOSE AND s_T DECIDES IT.** Required delta is **+0.025 to +0.053**: below
   the s_T~0.0134 crossover the +0.025 FLOOR governs (bar 0.6435, BELOW the lever's own
   effect); in the D18/D23/D25 spread range the bar is 0.650-0.672, ABOVE it. The anneal's
   horizon-matched effect is **+0.0277 at 12M** (`SESSION_LOGS.md:170`) — not DESIGN.md:101's
   +0.051, the 6M number. **P(CREDIT) 0.23-0.39 at typical spread, 0.60-0.75 if s_T lands
   low.** Six 12M arms have come in below 0.0236.
...
4. **Blocking build gate:** `ppo.py:999` (anneal × aux-head, param group 2) has **never
   executed** — no config pairs a live aux head with a live anneal, and no test combines
   them. (`tests/test_ppo.py:494` is NOT evidence: its agent builds no aux head.)
